...
interface = r"""SCOPESCRIPT PARAM_Preamble={preamble} PARAM_ScopeCode={scopecode} PARAM_CSharpCode={csharpcode} PARAM_OutputIsStructuredStream={outputisstructuredstream} PARAM_OutputSpecification={outputspecification} PATHIN_Input={input} PATHOUT_Output={output}.ss VC={vc} RETRIES=1 SCOPEPARAM=-p {jobpriority} -tokens {jobtokens} {nebulaarguments} """  
...
def sstreamprocessor_with_priority_tokens_nebula(
    ...
    csharpcode: str = "//",
    outputisstructuredstream: str = "true",
    outputspecification: str = "//",
):
    input_file_keys = ["input"]
    port_keys = ["input","output"]
...
if __name__ == '__main__':
    ComponentExecutor(sstreamprocessor_with_priority_tokens_nebula).execute(sys.argv)
