...
interface = r"""SCOPESCRIPT PATHOUT_Output={output}.ss PARAM_Start={start} PARAM_End={end} PARAM_Source={source} PARAM_PageType={pagetype} PARAM_Traffic={traffic} PARAM_ReturnHttps={returnhttps} VC=vc://{vc} RETRIES=2 SCOPEPARAM=-p {jobpriority} -tokens {jobtokens} {nebulaarguments} """  


@dsl._component(
    name="logrank_collect_gserp_clicks_from_xcuv",
    display_name="""[LogRank] Collect Gserp Clicks from xCUV""",
)
def logrank_collect_gserp_clicks_from_xcuv(
    output: OutputFile(),
    jobpriority: str,
    jobtokens: str,
    nebulaarguments: str,
    ...
    source: str = "\"DesktopIE\"",
    pagetype: str = "\"ALL\"",
    traffic: str = "\"Normal\"",
    returnhttps: str = "true",
...
if __name__ == '__main__':
    ComponentExecutor(logrank_collect_gserp_clicks_from_xcuv).execute(sys.argv)
