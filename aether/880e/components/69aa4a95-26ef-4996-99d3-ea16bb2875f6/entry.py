...
interface = r"""SCOPESCRIPT PATHIN_Input={input} PATHOUT_Output={output}.ss PARAM_V2RepoVersion={v2repoversion} VC=vc://{vc} RETRIES=2 SCOPEPARAM=-p {jobpriority} -tokens {jobtokens} {nebulaarguments} """  


@dsl._component(
    name="get_urlmetadata_and_dscmetadata_from_urlrepo",
    display_name="""Get UrlMetadata And DscMetadata From URLRepo""",
)
def get_urlmetadata_and_dscmetadata_from_urlrepo(
...
if __name__ == '__main__':
    ComponentExecutor(get_urlmetadata_and_dscmetadata_from_urlrepo).execute(sys.argv)
