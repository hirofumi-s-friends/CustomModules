...
interface = r"""Command=Copy OriginType=Resource Origin={inputresource} DestinationType=Cosmos DestinationOutput={cosmospath}  DestinationRoot={destinationroot} ShouldRespectLineBoundaries=[{shouldrespectlineboundaries}] """  


@dsl._component(
    name="copy_intermediate_directory_or_file_to_cosmos",
    display_name="""Copy Intermediate Directory or File to Cosmos""",
)
def copy_intermediate_directory_or_file_to_cosmos(
    inputresource: InputFile(type=(['AnyDirectory', 'AnyFile'])),
    cosmospath: OutputFile(),
    destinationroot: str,
    shouldrespectlineboundaries: str = "false",
):
    input_file_keys = ["inputresource"]
    port_keys = ["inputresource","cosmospath"]
...
if __name__ == '__main__':
    ComponentExecutor(copy_intermediate_directory_or_file_to_cosmos).execute(sys.argv)
