...
interface = r"""python CreateCosmosPath.py {vc} {tmppathprefix} """  


@dsl._component(
    name="create_cosmos_path",
    display_name="""Create cosmos path""",
)
def create_cosmos_path(
    tmppathprefix: OutputFile(),
    vc: str = "cosmos08/EntityMatching.Exp",
):
    input_file_keys = []
    port_keys = ["tmppathprefix"]
...
if __name__ == '__main__':
    ComponentExecutor(create_cosmos_path).execute(sys.argv)
