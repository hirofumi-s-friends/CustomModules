...
interface = r"""SCOPESCRIPT PATHIN_Input1={input1} PATHIN_Input2={input2} PATHOUT_Output={output}.ss PARAM_Preamble={preamble} PARAM_ScopeCode={scopecode} PARAM_OutputIsStructuredStream={outputisstructuredstream} PARAM_OutputSpecification={outputspecification} PARAM_CSharpCode={csharpcode} VC=vc://{vc} RETRIES=1 SCOPEPARAM=-p {jobpriority} -tokens {jobtokens} {nebulaarguments} """  


@dsl._component(
    name="sstreamprocessor2_with_priority_tokens_nebula",
    display_name="""SStreamProcessor2 with Priority Tokens Nebula""",
)
def sstreamprocessor2_with_priority_tokens_nebula(
    input1: InputFile(type=(['AnyDirectory', 'AnyFile'])),
    input2: InputFile(type=(['AnyDirectory', 'AnyFile'])),
    output: OutputFile(),
    preamble: str,
    scopecode: str,
    outputisstructuredstream: str,
    ...
    input_file_keys = ["input1","input2"]
    port_keys = ["input1","input2","output"]
...
if __name__ == '__main__':
    ComponentExecutor(sstreamprocessor2_with_priority_tokens_nebula).execute(sys.argv)
