...
from examples.args_util import predict_args
from examples.utils_squad import (convert_examples_to_features, RawResult,
                                  write_predictions, RawResultExtended,
...
class QA:
    ...
    def predict(self, input_data):
        ...
        dataset, examples, features = load_and_cache_examples(
            examples, self.max_seq_length, self.max_query_length,
            self.tokenizer)

        all_results = evaluate(device, self.model_config, dataset, features,
                               self.model)
        ...
        if self.model_config['model_type'] in ['xlnet', 'xlm']:
            ...
            predictions = write_predictions_extended(
                examples, features, all_results, 1, self.max_answer_length,
                input_data, self.model.config.start_n_top,
                self.model.config.end_n_top, self.tokenizer)
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            inputs = {
                ...
                batch[0],
                'token_type_ids':
                None if model_config['model_type'] == 'xlm' else batch[1],
...
def load_and_cache_examples(examples, max_seq_length, max_query_length,
                            tokenizer):
...
def main():
    ...
    all_results.to_parquet(
        os.path.join(args.output_dir, 'data.dataset.parquet'))
